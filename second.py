...
args = {
    ...
    'start_date': dt.datetime(2021, 02, 11),  # Время начала выполнения пайплайна
...
def mean_fare_per_class():
    titanic_df_mean = pd.read_csv(get_path('titanic.csv'))
    titanic_df_mean.regisreTempTable('titanic_df_mean')
    df_mean = spark.sql("""
                        select
                            Pclass
                            , avg(Fare)
...
with DAG(
    ...
    #еще одно
    mean_fare_per_class = PythonOperator(
        task_id='mean_fares_titanic_dataset',
        python_callable=mean_fares_titanic_dataset,
        dag=dag,
    )
    # Порядок выполнения тасок
    first_task >> create_titanic_dataset >> pivot_titanic_dataset >> mean_fare_per_class